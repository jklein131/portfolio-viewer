...
import { Position } from "@/components/DashboardTable/types";
import type { NextApiRequest, NextApiResponse } from "next";
import { getOptionsForTicker } from "./options";
...
export default async function handler(
  ...
  // let's get the options value:

  res.status(200).json(
    await Promise.all(
      mockData.map(async (t) => {
        if (t.price === null && t.assettype === "options") {
          const options = await getOptionsForTicker(t.underlying);
          const call = options.options.options.calls.find(
            (c) => c.contractSymbol === t.ticker
          );
          const put = options.options.options.puts.find(
            (c) => c.contractSymbol === t.ticker
          );
          console.log({ call, put });
          if (call) {
            return { ...t, price: call.optionValue };
          }
          if (put) {
            return { ...t, price: put.optionValue };
          }
        }
        return t;
      })
